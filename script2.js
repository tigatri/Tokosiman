let GOOGLE_APPS_SCRIPT_URL = localStorage.getItem('tokosiman_gas_url') || "https://script.google.com/macros/s/AKfycbzx9p6K-DBXFgNhTvA-xiVIGmBa6IR8hb3qLIih5b9avnicKgKQhLiNS8FFw4cv9Jyi/exec"; 
  ...
  let currentPinInput = "";
  // Variabel confirmOkCallback dihapus karena tidak digunakan (dead variable)
        ...
        <div class="py-3 border-b border-slate-200 last:border-b-0">
          <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-1.5">
              ...
              <button onclick="hapusKategoriUtama('${mainKat}')" class="w-7 h-7 bg-rose-50/90 backdrop-blur-sm text-rose-600 rounded-xl flex items-center justify-center border border-rose-200 hover:bg-rose-100 transition-all no-std-btn shadow-xs" title="Hapus">
  ...
  async function checkoutKasir() {
    ...
    setTimeout(async () => {
      ...
      // Diperbaiki: Langsung menggunakan waktu saat ini karena 'kasirDate' tidak ada di HTML
  ...
  async function submitProduk() {
    ...
    showLoading("Menyimpan produk...", "Mengirim data ke Google Sheets");
  ...
  async function submitForm() {
    ...
    let tanggal = document.getElementById('formTanggal').value;
    ...
    if (jenis === 'pemasukan') {
      ...
      pengeluaranVal = jumlah;
      modal = 0;
      untung = 0;
    } else if (jenis === 'hutang_saya') {
      pemasukanVal = jumlah;
      modal = 0;
      untung = 0;
      status = 'Belum Lunas';
    ...
    showLoading("Menyimpan catatan...", "Mengirim data ke Google Sheets");
    await dbSaveTransaction(payload);
    hideLoading();
  ...
  window.addEventListener('DOMContentLoaded', () => {
    let savedRemember = localStorage.getItem('tokosiman_remember_pin');
    if (savedRemember === 'true') {
      hideSplashLogin();
    }

    let initialTab = window.location.hash ? window.location.hash.substring(1) : 'beranda';
    if (['beranda', 'kasir', 'produk', 'laporan', 'setelan'].includes(initialTab)) {
    ...
    setInterval(() => {
      ...
      let clocks = ['liveClockBeranda', 'liveClockKasir', 'liveClockProduk', 'liveClockLaporan', 'liveClockSetelan'];
      clocks.forEach(id => {
        let el = document.getElementById(id);
