    ...
    function renderProdukList() {
      ...
      listFav.innerHTML = favs.length === 0 ? `<div class="py-2 text-slate-400 text-xs text-center">Tidak ada produk favorit</div>` : favs.map(p => {
        ...
        return `
          <div onclick='addToCart(${JSON.stringify(p)})' class="py-2.5 flex justify-between items-center text-xs cursor-pointer hover:bg-slate-50 transition-colors">
            ...
            <button class="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-semibold text-[10px] shadow-sm no-std-btn">Pilih</button>
          </div>
        `;
      }).join('');

      listOth.innerHTML = paginatedOthers.length === 0 ? `<div class="py-2 text-slate-400 text-xs text-center">Tidak ada produk</div>` : paginatedOthers.map(p => {
        let jual = Number(p.hargaJual !== undefined ? p.hargaJual : (p.harga || 0));
        let modal = Number(p.hargaBeli !== undefined ? p.hargaBeli : (p.modal || 0));
        let untung = jual - modal;
        return `
      ...
      document.getElementById('modalProdukPageInfo').innerText = `Hal ${modalProdukPage} dari ${totalPages}`;
      document.getElementById('modalProdPrevBtn').disabled = (modalProdukPage <= 1);
      document.getElementById('modalProdNextBtn').disabled = (modalProdukPage >= totalPages);
    }
    ...
    function renderKasirProdukList() {
      ...
      if (paginated.length === 0) {
        grid.innerHTML = `<div class="col-span-2 text-center py-6 text-slate-400 text-xs">Tidak ada produk ditemukan</div>`;
      ...
      grid.innerHTML = paginated.map(p => {
        ...
        let untung = jual - modal;
        return `
          <div onclick='addToKasirCart(${JSON.stringify(p)})' class="modern-card p-3 flex flex-col justify-between cursor-pointer hover:border-emerald-300 transition-all active:scale-[0.98] shadow-xs">
            <div>
              <div class="font-bold text-slate-900 text-xs line-clamp-2">${p.nama}</div>
              <div class="text-[9px] text-slate-500 mt-0.5">${p.kategori || 'Umum'}</div>
            </div>
            <div class="pt-2 mt-2 border-t border-slate-200/80 flex justify-between items-end">
              <div>
                <div class="font-black text-emerald-700 text-xs">${formatRp(jual)}</div>
                <div class="text-[8px] text-slate-500 font-semibold">+${formatRp(untung)}</div>
              </div>
              <div class="w-6 h-6 rounded-lg bg-emerald-600 text-white flex items-center justify-center text-xs shadow-md shadow-emerald-600/30">
    ...
    function renderProdukTabList() {
      ...
      if (listLainnya) {
        listLainnya.innerHTML = paginatedLainnya.length === 0 ? `<div class="py-4 text-center text-slate-400 text-xs">Tidak ada produk</div>` : paginatedLainnya.map(p => {
          let jual = Number(p.hargaJual !== undefined ? p.hargaJual : (p.harga || 0));
          let modal = Number(p.hargaBeli !== undefined ? p.hargaBeli : (p.modal || 0));
          let untung = jual - modal;
          return `
            <div class="py-2.5 flex justify-between items-center text-xs">
              <div class="flex-1 pr-2" onclick='openFormEditProduk(${JSON.stringify(p)})'>
                <div class="font-bold text-slate-900 cursor-pointer hover:underline">${p.nama}</div>
                <div class="text-[10px] text-slate-500 mt-0.5"><span class="text-emerald-700 font-bold">${formatRp(jual)}</span> • Modal ${formatRp(modal)} (Untung <span class="text-emerald-700 font-semibold">${formatRp(untung)}</span>)</div>
              </div>
              <div class="flex items-center gap-1">
                <button onclick='openFormEditProduk(${JSON.stringify(p)})' class="w-7 h-7 bg-white/80 backdrop-blur-sm text-indigo-700 rounded-lg flex items-center justify-center border border-slate-200 hover:bg-slate-100 transition-all no-std-btn shadow-xs"><i class="fa-solid fa-pen text-xs"></i></button>
                <button onclick='hapusProdukOtomatis("${p.id}")' class="w-7 h-7 bg-rose-50/90 backdrop-blur-sm text-rose-600 rounded-lg flex items-center justify-center border border-rose-200 hover:bg-rose-100 transition-all no-std-btn shadow-xs"><i class="fa-solid fa-trash text-xs"></i></button>
              </div>
            </div>
          `;
        }).join('');
      ...
      if (listFavorit) {
        listFavorit.innerHTML = paginatedFavorit.length === 0 ? `<div class="py-4 text-center text-slate-400 text-xs">Tidak ada produk favorit</div>` : paginatedFavorit.map(p => {
          let jual = Number(p.hargaJual !== undefined ? p.hargaJual : (p.harga || 0));
          let modal = Number(p.hargaBeli !== undefined ? p.hargaBeli : (p.modal || 0));
          let untung = jual - modal;
          return `
        ...
        document.getElementById('infoPaginationFavorit').innerText = `Hal ${produkPageFavorit} dari ${totalPagesFavorit}`;
        document.getElementById('btnPrevFavorit').disabled = (produkPageFavorit <= 1);
        document.getElementById('btnNextFavorit').disabled = (produkPageFavorit >= totalPagesFavorit);
      }
    }
    ...
    function openFormEditProduk(p) {
      ...
      renderKategoriDropdown();
      let katSelect = document.getElementById('prodKategori');
      if (katSelect && p.kategori) {
        katSelect.value = p.kategori;
        onProdKategoriChange(p.kategori);
      }
      let subSelect = document.getElementById('prodSubKategori');
      if (subSelect && p.subKategori) {
        subSelect.value = p.subKategori;
      }
    ...
    async function submitProduk() {
      ...
      showLoading("Menyimpan produk...", "Menyinkronkan ke Google Sheets");
    ...
    function hapusProdukOtomatis(id) {
      showConfirm("Hapus Produk", "Apakah Anda yakin ingin menghapus produk ini?", function() {
        showLoading("Menghapus produk...");
        dbDeleteProduct(id).then(() => {
          hideLoading();
    ...
    async function submitForm() {
      ...
      let tanggal = document.getElementById('formTanggal').value || getLocalDateString();

      if (jumlah <= 0) {
        showToast("Jumlah nominal wajib diisi lebih besar dari Rp0!", "error");
        return;
      }

      if ((status === 'Belum Lunas' || jenis === 'hutang_saya') && !namaPelanggan) {
        showToast("Nama pihak / pelanggan wajib diisi untuk catatan hutang!", "error");
        return;
      }
      ...
      let payload = {
        ...
        jenis: jenis,
        status: status,
        modeBayar: status === 'Belum Lunas' ? 'Hutang' : 'Tunai',
        ...
        bayar: status === 'Belum Lunas' ? 0 : jumlah,
        kembalian: 0,
      ...
      showLoading("Menyimpan catatan...", "Menyinkronkan transaksi ke Google Sheets");
      await dbSaveTransaction(payload);
      hideLoading();
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      setInterval(() => {
        ...
        ['Beranda', 'Kasir', 'Produk', 'Laporan', 'Setelan'].forEach(tab => {
          let el = document.getElementById('liveClock' + tab);
